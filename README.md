...
> (../notes/synapcores-quirks.md) if you hit unexpected errors.
...
- **REST AutoML instead of `CREATE EXPERIMENT` SQL.** The brief asked for the SQL extensions, and
  ...
  (../notes/synapcores-quirks.md) for the full diagnosis). The REST `/v1/automl/*` endpoints respond
...
- **The `SYNAPCORES_USE_SQL_AUTOML=true` path is implemented in the SDK shape but not exercised
  ...
  [synapcores-quirks.md](../notes/synapcores-quirks.md)), so this path was quarantined rather than
  shipped broken.
...
it. Full diagnosis is in [notes/synapcores-quirks.md](../notes/synapcores-quirks.md).

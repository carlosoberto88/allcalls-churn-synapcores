...
- **The live AutoML pipeline cannot complete on this CE build — verified through the vendor's
  own UI.** Driven the SynapCores web UI with Playwright on the vendor's own *Customer Churn
  Prediction* recipe and captured the network request: even the official client hits
  `{"error":{"code":"query_error","message":"Operation timeout"}}` on the recipe's first
  `CREATE TABLE`. Same outcome from REST AutoML — `POST /v1/automl/datasets/{id}/upload`
  accepts the body but `row_count` never moves off `0`. The SDK was updated to match the
  vendor UI's actual `/v1/query/execute` body shape (`max_rows`, `timeout_secs`, `parameters`,
  no `database`); the SQL engine still doesn't execute. Full request/response capture is in
  [docs/synapcores-quirks.md](docs/synapcores-quirks.md) under *Live-traffic verification*.
